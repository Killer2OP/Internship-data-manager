...
import { FNRDialog } from "./FNRDialog"; // Import FNRDialog

export default function Header() {
  const router = useRouter()
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isFNRDialogOpen, setIsFNRDialogOpen] = useState(false); // State for FNR dialog
  ...
  const handleSubmitFNRClick = () => {
    setIsFNRDialogOpen(true); // Open the FNR dialog
  }
  ...
  return (
    <header className="bg-gray-200 text-gray-800 p-4 shadow-md">
            ...
            <ul className="flex space-x-4">
              ...
              <li>
                <Button 
                  variant="ghost" 
                  className="text-gray-800 hover:text-primary hover:bg-gray-300"
                  onClick={handleSubmitFNRClick} // Add onClick handler for FNR
                >
      ...
      />
      <FNRDialog 
        open={isFNRDialogOpen}
        setOpen={setIsFNRDialogOpen}
      />
    </header>
  )
}